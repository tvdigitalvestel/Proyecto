        ...
        <tr>
            <td >
                ...
                if ($invoice['company']) echo $invoice['company'] . '<br>';
                 echo "Direction " .': '.$direccionx
					 .'<br>Apartment'. $this->lang->line('referencia') . ' : ' . $invoice['referencia'];
					 //.'<br>Concept: ' . $concept
            ...
            if(count($lista_invoices)>0 || $is_multiple){
                ...
                if(count($transacciones)!=0){                    
                    ...
                    foreach ($lista_items as $key => $value) {
                        ...
                        //PARA EL NOMBRE DEL MES EN LA FACTURA
                        echo '<tr class="item' . $flag . '"> 
                                <td>' . strftime("%B", strtotime($f1)). ' CTA:'. $invoice['tid'].'</td>';
...
/* end codigo pagos adelantados*/
  ...
  if ($invoice['shipping'] > 0) { $cols++;}
    ...
    echo '</strong></td>
        ...
        <div class="sign2">(' . $user . ')</div><div class="sign3">' . user_role($perfil) . '</div> 
        <div class="terms"><hr><strong>' . $this->lang->line('Terms') . ':</strong>';
